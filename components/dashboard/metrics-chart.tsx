...
export function MetricsChart({ title, description, color, metricType = 'response_time' }: MetricsChartProps) {
  ...
  const dataStableRef = useRef<Array<any>>([])
  ...
  const generateSampleData = useCallback(() => {
    // Prevent unnecessary regeneration if data is stable
    const now = Date.now()
    if (dataStableRef.current.length > 0 && now - lastUpdateTime.current < 300000) {
      return
    }
    ...
    for (let i = 19; i >= 0; i--) {
      const time = new Date(currentTime.getTime() - (i * 5 * 60 * 1000)) // Every 5 minutes
      const dataPoint: any = {
    ...
    dataStableRef.current = sampleData
    setData(sampleData)
  }, [metricType, services])
  ...
  const loadServicesAndMetrics = useCallback(async () => {
    try {
      ...
      try {
        ...
        console.log('Metrics API not available, using sample data')
        generateSampleData()
      }
    } catch (error) {
      console.log('Using fallback sample data due to API issues')
      generateSampleData()
    } finally {
      setIsLoading(false)
    }
  ...
  // Update with real-time data when new metrics arrive (very heavily throttled)
  useEffect(() => {
    if (latestMetrics.length > 0 && services.length > 0) {
      const now = Date.now()
      // Throttle updates to every 2 minutes to completely prevent blinking
      if (now - lastUpdateTime.current > 300000) {
      ...
      <Card className="border-0 shadow-sm">
        ...
        <CardContent>
          <div className="h-64 flex items-center justify-center">
            <div className="text-muted-foreground">Loading metrics chart...</div>
          </div>
        </CardContent>
      </Card>
    )
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
                    ...
                    return (
                      <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
                        ...
                        {payload.map((entry, index) => (
                          <p key={`tooltip-${title}-${index}-${entry.dataKey}`} className="text-sm" style={{ color: entry.color }}>
              ...
              {serviceNames.map((serviceName, index) => (
                <Line
                  key={`${title}-${serviceName}-${metricType}-${index}`}
                  type="monotone"
                  dataKey={serviceName}
