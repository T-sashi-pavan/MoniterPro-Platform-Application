...
export function MetricsChart({ title, description, color, metricType = 'response_time' }: MetricsChartProps) {
  ...
  const dataBuffer = useRef<Array<any>>([])
  ...
  const loadServicesAndMetrics = useCallback(async () => {
    try {
      ...
      metrics.forEach(metric => {
        const timeKey = new Date(metric.lastChecked).toLocaleTimeString("en-US", {
          hour12: false,
          hour: "2-digit",
          minute: "2-digit",
        })
        ...
        const service = servicesData.find(s => s._id === metric.serviceId)
        const serviceName = service?.name || `Service ${metric.serviceId}`
  ...
  return (
    <Card className="border-0 shadow-sm">
      ...
      metrics.forEach(metric => {
        const timeKey = new Date(metric.lastChecked || metric.last_checked || Date.now()).toLocaleTimeString("en-US", {
          hour12: false,
          hour: "2-digit",
          minute: "2-digit",
        })
  ...
  // Get unique service names from data for legend
  const serviceNames = data.length > 0 ? 
