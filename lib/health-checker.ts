...
import { HealthCheckResult, Service, ServiceMetric } from '../lib/types';

export class HealthChecker {
  private checkTimeout = 10000; // 10 seconds timeout
  ...
  async checkAllServices(): Promise<HealthCheckResult[]> {
    try {
      const { rows: services } = await pool.query<Service>('SELECT * FROM services');
  ...
  async saveMetrics(results: HealthCheckResult[]): Promise<void> {
    ...
    try {
      ...
      for (const result of results) {
        await client.query(
          `INSERT INTO service_metrics (service_id, status, response_time, cpu_usage, memory_usage, last_checked) 
           VALUES ($1, $2, $3, $4, $5, $6)`,
          [result.service_id, result.status, result.response_time, null, null, new Date()]
        );
        
        // Log errors if service is offline
        if (result.status === 'offline' && result.error) {
          await client.query(
            `INSERT INTO logs (service_id, level, message, timestamp) 
             VALUES ($1, $2, $3, $4)`,
            [result.service_id, 'error', `Service check failed: ${result.error}`, new Date()]
          );
        }
      }
  ...
  async getLatestMetrics(): Promise<ServiceMetric[]> {
    try {
      const { rows } = await pool.query<ServiceMetric>(`
  ...
  async getMetricsHistory(serviceId?: number, hours: number = 24): Promise<ServiceMetric[]> {
    try {
